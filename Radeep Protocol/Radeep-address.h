#ifndef RADEEP_ADDRESS_H
#define RADEEP_ADDRESS_H
 ...
 * \ingroup address
 *
 * \brief Radeep addresses are stored in host order in this class.
 *
 * \see attribute_RadeepAddress
 */
class RadeepAddress {
public:
  RadeepAddress ();
   ...
   */
  explicit RadeepAddress (uint32_t address);
  /** 
    * \brief Constructs an RadeepAddress by parsing a the input C-string
    *
    ...
    */
  RadeepAddress (char const *address);
  /** 
   * Get the host-order 32-bit Radeep address
   * \return the host-order 32-bit Radeep address
   */
  ...
  /** 
    * \brief Sets an RadeepAddress by parsing a the input C-string
    *
    * Input address is in format:
    * \c hhh.xxx.xxx.lll
    * where \c h is the high byte and \c l the
  ...
  /**
   * \brief Comparison operation between two RadeepAddresses
   * \param other address to which to compare this address
   * \return True if the addresses are equal. False otherwise.
   */
  bool IsEqual (const RadeepAddress &other) const
  {
   ...
   * \return an RadeepAddress
   * 
   * The input address is expected to be in network byte order format.
   */
  static RadeepAddress Deserialize (const uint8_t buf[4]);
  /**
   * \brief Print this address to the given output stream
   *
   * The print format is in the typical "192.168.1.1"
   * \param os The output stream to which this RadeepAddress is printed
   */
   ...
   *
   * This method returns an Radeep address that is this address combined
   * (bitwise and) with a network mask, yielding an Radeep network
   * address.
   *
   * \param mask a network mask 
   * \returns the address combined with the mask
   */
  RadeepAddress CombineMask (RadeepMask const &mask) const;
  /**
   ...
   */
  RadeepAddress GetSubnetDirectedBroadcast (RadeepMask const &mask) const;
  /**
   * \brief Generate subnet-directed broadcast address corresponding to mask
   * 
   * The subnet-directed broadcast address has the host bits set to all
   ...
   */
  bool IsSubnetDirectedBroadcast (RadeepMask const &mask) const;
  /**
   ...
   * \return a new RadeepAddress from the polymorphic address
   *
   * This function performs a type check and asserts if the
   * type of the input address is not compatible with an
   * RadeepAddress.
   */
  static RadeepAddress ConvertFrom (const Address &address);
  /**
   * \return the 0.0.0.0 address
   */
  static RadeepAddress GetZero (void);
  /**
   * \return the 0.0.0.0 address
   */
  static RadeepAddress GetAny (void);
  /**
   * \return the 255.255.255.255 address
   */
  static RadeepAddress GetBroadcast (void);
  /**
   * \return the 127.0.0.1 address
   */
  static RadeepAddress GetLoopback (void);
...
private:
  ...
  uint32_t m_address; //!< Radeep address
   ...
   */
  friend bool operator == (RadeepAddress const &a, RadeepAddress const &b);
 ...
 * \ingroup address
 *
 * \brief a class to represent an Radeep address mask
 * 
 * The constructor takes arguments according to a few formats. 
 * RadeepMask ("255.255.255.255"), RadeepMask ("/32"), and RadeepMask (0xffffffff)
 * are all equivalent.
 *
 * \see attribute_RadeepMask
 */
class RadeepMask {
public:
  /**
   ...
   */
  bool IsMatch (RadeepAddress a, RadeepAddress b) const;
  /**
   * \param other a mask to compare 
   * \return true if the mask equals the mask passed as input parameter
   */
  bool IsEqual (RadeepMask other) const;
  /** 
   * Get the host-order 32-bit Radeep mask
   * \return the host-order 32-bit Radeep mask
   */
  uint32_t Get (void) const;
  /**
   ...
   */
  static RadeepMask GetLoopback (void);
  /**
   * \return the 0.0.0.0 mask
   */
  static RadeepMask GetZero (void);
  /**
   * \return the 255.255.255.255 mask
   */
  static RadeepMask GetOnes (void);

private:
  uint32_t m_mask; //!< Radeep mask
};

ATTRIBUTE_HELPER_HEADER (RadeepAddress);
ATTRIBUTE_HELPER_HEADER (RadeepMask);
 ...
 */
std::ostream& operator<< (std::ostream& os, RadeepAddress const& address);
/**
 * \brief Stream insertion operator.
 *
 * \param os the stream
 ...
 * \returns a reference to the stream
 */
std::ostream& operator<< (std::ostream& os, RadeepMask const& mask);
/**
 ...
 * \returns a reference to the stream
 */
std::istream & operator >> (std::istream &is, RadeepAddress &address);
/**
 * \brief Stream extraction operator.
 *
 * \param is the stream
 ...
 * \returns a reference to the stream
 */
std::istream & operator >> (std::istream &is, RadeepMask &mask);
 ...
 * \returns true if the operands are equal
 */
inline bool operator == (const RadeepAddress &a, const RadeepAddress &b)
{
 ...
 * \ingroup address
 *
 * \brief Class providing an hash for Radeep addresses
 */
class RadeepAddressHash : public std::unary_function<RadeepAddress, size_t> {
public:
  /**
   ...
   * \return the hash
   */
  size_t operator() (RadeepAddress const &x) const;
};
 ...
 * \returns true if the operands are equal
 */
bool operator == (RadeepMask const &a, RadeepMask const &b);
/**
 * \brief Not equal to operator.
 *
 * \param a the first operand
