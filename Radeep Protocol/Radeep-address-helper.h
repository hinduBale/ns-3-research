#ifndef RADEEP_ADDRESS_HELPER_H
#define RADEEP_ADDRESS_HELPER_H
...
class RadeepAddressHelper
{
public:
/**
 * @brief Construct a helper class to make life easier while doing simple Radeep
 * address assignment in scrRadeepts.
 */
  RadeepAddressHelper ();
 ...
 */
  RadeepAddressHelper (RadeepAddress network, RadeepMask mask, 
                     RadeepAddress base = "0.0.0.1");
 ...
 *
 * @param network The RadeepAddress containing the initial network number to
 * use during allocation.  The bits outside the network mask are not used.
 * @param mask The RadeepMask containing one bits in each bit position of the 
 * network number.
 * @param base An optional RadeepAddress containing the initial address used for 
 * IP address allocation.  Will be combined (ORed) with the network number to
 * generate the first IP address.  Defaults to 0.0.0.1.
 * @returns Nothing.
 */
  void SetBase (RadeepAddress network, RadeepMask mask, 
                RadeepAddress base = "0.0.0.1");
 ...
 * looks up the Radeep interface corresponding to the net device.  It then sets
 * the RadeepAddress and mask in the interface to the appropriate values.  If 
 ...
 * are asked to assign Radeep addresses to.
 *
 * @returns A container holding the added NetDevices
 * @see SetBase
 * @see NewNetwork
 */
  RadeepInterfaceContainer Assign (const NetDeviceContainer &c);
