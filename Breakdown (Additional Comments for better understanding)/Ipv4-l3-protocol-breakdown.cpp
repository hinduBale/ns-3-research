...
Therefore,we could very well remove the "advanced" trace sources such as the Drop, SendOutgoing, etc.

 We need not concern ourselves with fragmentation and reassembly, and therefore we will neglect\
 its implementation in our Radeep Protocol. 
 */


class Ipv4L3Protocol : public Ipv4   //Base class is Ipv4
{
public:
  
//typeid is a C++ language operator which returns type
// identification information at run time. It basically returns a type_info object,
// which is equality-comparable with other type_info objects.

  static TypeId GetTypeId (void);
  static const uint16_t PROT_NUMBER; //The protocol number is to be set at 99 for our new custom protocol/
  ...
  //But, since our new custom protocol, is going to be quite rudimentry, we can altogether, neglect it.
  //enum DropReason 
  //{
  ...
  //Creates a new Raw Socekt (A network socket is an internal endpoint for sending/recieving
  //data within a node on a computer network.)
  Ptr<Socket> CreateRawSocket (void);
  void DeleteRawSocket (Ptr<Socket> socket);

  // Registers a working L4 Protocol which can be used on the node.
  ...
  //Removes the above L4 protocols
